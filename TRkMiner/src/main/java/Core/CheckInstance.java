...
public class CheckInstance {
    ...
    private static Boolean verifyTwoNodeInGraph(Graph G, Pattern newPattern, PNode newInNode, PNode newOutNode) {
        ...
        HashSet<Integer> noDelIDList = new HashSet<>();
        ...
        if (Utils.isFrequent(noDelIDList)) {
            newInNode.setInstanceIDSet(noDelIDList);
            return true;
        }
        return false;
    }
    ...
    private static Boolean search(Graph G, ArrayList<PNode> pathList, int nodeIndex, int id, int startIndex,HashMap<Integer, Boolean> visit) {
        if (visit.containsKey(id)) {
            return visit.get(id);
        }
        if (pathList.size() == nodeIndex) {
            boolean flag = G.containsEdge(G.getInstanceByID(startIndex), G.getInstanceByID(id));
            visit.put(id, flag);
            return flag;
        }
        boolean flag = true;
...
//            System.out.println(i);
            if (G.containsEdge(G.getInstanceByID(i), G.getInstanceByID(id)) && search(G, pathList, nodeIndex + 1, i, startIndex,visit)) {
                visit.put(id, true);
                flag = false;
//                return true;
            }
