...
public class NewGetInfoFromGraph {
    static Logger logger = Logger.getLogger(NewGetInfoFromGraph.class.getName());
    static TopKList topKPattern = NewTopRankK.topKPatternList;
    /**
...
//                        v.get(1).addAll(temp);
                    ...
                    if(pattern.getMaxItrInForward() >= topKPattern.getMinimumItr()) {
    ...
    public static FrequentEdgePatternResult getAllEdgePattern(Graph G) {
        ...
        labelEdge.forEach((inLabel, outLabels) -> {
            ...
            outLabels.forEach((outLabel, v) -> {
                ...
                //开始维护topK
//                NewTopRankK.topKPatternList.insert(pattern);
                frequentEdgePattern.add(pattern);
            });
        });
